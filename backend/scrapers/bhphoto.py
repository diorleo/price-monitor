"""
scrapers/bhphoto.py — B&H Photo 爬虫（v2 重写）

改进点：
1. 多搜索词 + 分页：MOZA RACING (2页) + MOZA (1页)
2. 智能产品匹配（共享 match_product 方法）
3. 多策略解析：JSON-LD → CSS选择器 → 正则回退
4. 注意：B&H 不销售 MOZA RACING 模拟赛车产品，
   搜索 "moza" 返回的是 MOZA 品牌音频设备。
...
class BHPhotoScraper(BaseScraper):
    ...
    SEARCH_URL = "https://www.bhphotovideo.com/c/search?q={query}&sts=at&settingItemsPerPage=48"
    ...
    def scrape(self) -> list[dict]:
        ...
        for query, max_pages in self.SEARCH_QUERIES:
            for page in range(1, max_pages + 1):
                ...
                if not html:
                    logger.warning(f"[B&H] Failed to fetch: {query} page {page}")
                    continue

                page_products = self._parse_search_results(html)
                if not page_products:
                    logger.info(f"[B&H] No products on {query} page {page}, stopping")
                    break

                all_products.extend(page_products)
                logger.info(f"[B&H] {query} page {page}: found {len(page_products)} products")
        ...
        logger.info(f"[B&H] Total unique products after dedup: {len(unique)}")

        fallback_url = self.SEARCH_URL.format(query="moza+racing")
        if not unique:
            logger.warning("[B&H] No products found, returning all Missing")
    ...
    def _extract_from_json_ld(self, html: str) -> list[dict]:
        ...
        try:
            ...
            logger.debug(f"[B&H] JSON-LD extraction failed: {e}")
    ...
    def _extract_from_html(self, html: str) -> list[dict]:
        ...
        item_selectors = [
            ...
            'div[data-selenium="itemContainer"]',
            'div.product-info',
        ...
        # 备用：找产品链接
        if not items:
            links = soup.select('a[href*="/bhphoto/"]') or soup.select('a[href*="/c/product/"]')
            for link in links:
    ...
    def _parse_html_item(self, item) -> dict | None:
        ...
        link_selectors = [
            'a[data-selenium="itemLink"]',
            'a[data-selenium="productLink"]',
            'a.title',
            'a[href*="/c/product/"]',
            'a[href*="/bhphoto/"]',
        ]
        for sel in link_selectors:
            ...
            if el:
                ...
                href = el.get('href', '')
                if href:
                    url = ('https://www.bhphotovideo.com' + href) if href.startswith('/') else href
                break

        if not title:
            title_el = item.select_one('[data-selenium="itemTitle"], .title, .product-title, h2, h3')
            if title_el:
    ...
    def _extract_with_regex(self, html: str) -> list[dict]:
        ...
        # B&H 产品链接格式
        pattern = re.compile(
            r'href="(/c/product/[^"]+)"[^>]*>\s*(?:<[^>]+>)*\s*([^<]+)',
            re.IGNORECASE
        )
        for match in pattern.finditer(html):
            href = match.group(1)
            title = match.group(2).strip()
            url = 'https://www.bhphotovideo.com' + href
